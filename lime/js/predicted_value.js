...
class PredictedValue {
  ...
  constructor(
    ...
    if (min_value == max_value) {
      ...
      var width_proportion =
        (predicted_value - min_value) / (max_value - min_value);
    }
    ...
    let num_digits =
      Math.floor(Math.max(Math.log10(min_value), Math.log10(max_value))) + 2;
    ...
    text
      .attr("x", bar_x + x_scale(width_proportion))
    ...
    text
      .attr("x", bar_x + bar_width + corner_padding)
      .attr("y", bar_y + bar_height - 3)
      .attr("fill", "black")
    ...
    this.svg_height =
      n_bars * bar_height + bar_yshift + 2 * text.node().getBBox().height + 10;
